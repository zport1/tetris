...
it checks for a file, if none exists it makes one
it adds and sorts scores and also creates blanks if there aren't enough scores
'''
import os
...
for i in range(len(scorage)-1):
    for j in range(1,len(scorage)):
        ...
        if scorage[-j][2] > scorage[-j-1][2]:
            #scan from the bottom up checking if second to last is smaller than last
